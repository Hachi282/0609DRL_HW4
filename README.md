...
## 🚀 部署至 GitHub Pages 步驟 (Deployment Guide)

你可以輕鬆將此 Demo 發布為公開的線上網站，供助教與評審直接操作：

1. **推送到你的 GitHub 倉庫**（已由 AI 自動推送到 `https://github.com/Hachi282/0609DRL_HW4.git`）。
2. 打開 GitHub 瀏覽器，進入你的倉庫頁面。
3. 點擊頂部的 **"Settings" (設定)** 索引標籤。
4. 在左側選單中找到並點擊 **"Pages"**。
5. 在 "Build and deployment" 下方的 **"Source"** 選擇 **"Deploy from a branch"**。
6. 在 **"Branch" (分支)** 下拉選單中，選擇 **`main`**，後方的目錄選擇 **`/(root)`**，然後點擊 **"Save" (儲存)**。
7. 等待 1-2 分鐘，GitHub 會自動建置網頁。重新整理頁面，頂部會出現發布成功的網址：`https://<你的GitHub帳號>.github.io/0609DRL_HW4/`。
...
- **輸入參數**：
  ...
  ```
- **功能**：**【確定性防護網】**。使用精確的程式算法計算最低毛利允許售價（$\text{Cost} \times (1 + \text{Margin})$）、競品中位數與均價，並根據定位（平價/中階/高檔）回傳最終定價區間。
...
- **利潤安全護欄 (Margin Protection)**：任何工具生成的建議定價均須通過商務公式檢驗：$\text{Price} \ge \text{Cost} \times (1 + \text{Margin})$。若計算器返回數值低於此線，系統會強制阻斷，將售價修正為毛利底線，並在報告中打上安全標記。
